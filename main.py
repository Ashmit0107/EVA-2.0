...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are EVA, an Enhanced Virtual Assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "EVA checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop EVA. "
...
class EvaLive:
    ...
    def __init__(self, ui: EvaUI):
        self.ui             = ui
        self._asst_name     = "EVA"   # updated each session from config
        self.session              = None
    ...
    def interrupt(self) -> None:
        """Stop EVA mid-speech: drain queued audio and open mic immediately."""
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[EVA] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                eva_speaking = self._is_speaking
            if not eva_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session:
                # Don't interrupt if user spoke recently or EVA is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[EVA] Connected.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: EVA online.")
                ...
                err_str = str(e)
                print(f"[EVA] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
            ...
            print(f"[EVA] Reconnecting in {delay}s...")
