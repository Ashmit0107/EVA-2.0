...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".eva_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".eva_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using EVA profile")
                eva_profile = str(Path.home() / ".eva_profiles" / "firefox_eva")
                Path(eva_profile).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(eva_profile, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".eva_profiles" / "safari")
        ...
        # EVA otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        eva_profile = str(Path.home() / ".eva_profiles" / self.browser_name)
        Path(eva_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with EVA profile: {eva_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(eva_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with EVA profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "eva_screenshot.png")
