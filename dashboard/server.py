"""
dashboard/server.py — EVA Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "EVA Uploads",
        Path.home() / "Documents" / "EVA Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"EVA Dashboard Port {port}"
        prog_rule  = "EVA Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="eva_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until EVA is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "eva.key").exists() and (certs / "eva.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in EVA to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('eva_token','{tok}');
  sessionStorage.setItem('eva_key','{key}');
  localStorage.setItem('eva_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to EVA…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "eva.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "eva.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in EVA UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
